...
export default async function handler(req, res) {
    ...
    try {
        // 弃用死掉的 ^W5000，改抓活的标普500 (^GSPC)
        const [yahooResp, fredResp] = await Promise.all([
            fetch('https://query1.finance.yahoo.com/v8/finance/chart/^GSPC?interval=1d&range=1d', { headers: { 'User-Agent': 'Mozilla/5.0' } }),
        ...
        if (!yahooResp.ok || !fredResp.ok) throw new Error("底层数据源获取失败");

        const yahooData = await yahooResp.json();
        const fredData = await fredResp.json();

        // 拿到标普 500 的最新点位
        const sp500 = yahooData.chart.result[0].meta.regularMarketPrice;
        const gdp = parseFloat(fredData.observations[0].value);

        // 核心修正：目前标普500的1个点，大约对应全市场 142 亿美元的总市值（乘数系数 14.2）
        const totalMarketCap = sp500 * 14.2; 
